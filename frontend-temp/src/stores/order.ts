...
export const useOrderStore = defineStore('order', {
  ...
  getters: {
    ...
    rentingCount: (s) => s.list.filter((o) => o.status === 'renting').length,
    returnedCount: (s) => s.list.filter((o) => o.status === 'returned').length,
    overdueCount: (s) => s.list.filter((o) => o.status === 'overdue').length,
    totalAmount: (s) => s.list.reduce((sum, o) => sum + o.amount, 0),
    /** 进行中订单（车辆归还页用） */
  ...
  actions: {
    async load() {
      this.loading = true
      try {
        const result = await api.orders.search({ page: 1, size: 999 })
