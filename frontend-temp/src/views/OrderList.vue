  ...
  <div class="paged-view">
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-4 gap-5">
        <StatCard label="订单总数" :value="store.total" suffix="单" icon="Tickets" tone="brand" />
        ...
        <StatCard label="已逾期" :value="store.overdueCount" suffix="单" icon="Warning" tone="orange" />
      </div>
      ...
      <GlassCard>
        ...
        <el-table-column prop="plateNo" label="车牌号码" min-width="105" />
        <el-table-column prop="customerName" label="客户姓名" min-width="80" />
        <el-table-column label="出租时间" min-width="100">
          <template #default="{ row }">{{ dateOnly(row.startDate) }}</template>
        </el-table-column>
        <el-table-column label="归还时间" min-width="100">
          <template #default="{ row }">
            <span v-if="row.returnTime">{{ dateOnly(row.returnTime) }}</span>
            <span v-else class="text-ink-muted">—</span>
          </template>
        </el-table-column>
        <el-table-column label="出租类型" min-width="120">
          <template #default="{ row }">
        ...
        <el-table-column label="操作" width="160">
          <template #default="{ row }">
            <div class="flex gap-2">
              <button class="flex h-8 items-center rounded-lg bg-brand/10 px-3 text-xs font-medium text-brand hover:bg-brand/20" @click="openDetail(row)">详情</button>
              <button
                v-if="row.depositReturn === 0 && row.returnTime"
                class="flex h-8 items-center rounded-lg bg-danger/10 px-3 text-xs font-medium text-danger hover:bg-danger/20"
