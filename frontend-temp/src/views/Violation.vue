    ...
    <PageHeader title="违章处理" subtitle="车辆违章记录登记与处理跟踪">
      <template #actions>
        <button class="btn-brand h-10 px-5 text-sm" @click="openAdd">
          <el-icon class="mr-1"><Plus /></el-icon>新增违章
        </button>
      </template>
    </PageHeader>

    <div class="grid grid-cols-3 gap-5">
      <StatCard label="违章总数" :value="store.list.length" suffix="条" icon="Warning" tone="brand" />
      <StatCard label="待处理" :value="store.pendingCount" suffix="条" icon="Bell" tone="orange" />
      <StatCard label="累计罚款" :value="store.list.reduce((s, v) => s + v.fine, 0)" suffix="元" icon="Money" tone="purple" />
    </div>
        ...
        <el-table-column label="操作" width="160" fixed="right">
          <template #default="{ row }">
            <div class="flex gap-2">
              <button v-if="row.status === 'pending'" class="flex h-8 w-8 items-center justify-center rounded-lg bg-statusOk/10 text-statusOk hover:bg-statusOk/20" title="处理" @click="handle(row)">
