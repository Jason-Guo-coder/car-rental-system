...
只读检查范围：老前端 `frontend/src/views/system/*` 与新前端 `frontend/src/views/system/*`，以及后端 `UserController`、`DeptController`、`RoleController`、`PermissionController`。本次只记录问题，不修改业务代码。
