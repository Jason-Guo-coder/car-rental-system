...
type RentalTypeSearchParams = Partial<RentalType> & { page?: number; size?: number; discountMin?: number; discountMax?: number }
type MaintainSearchParams = Partial<Maintain> & { page?: number; size?: number; costMin?: number; costMax?: number; lowMaintainTime?: string; highMaintainTime?: string }
...
const makerPage = (p: Page<AnyObj>): PageResult<Maker> => ({ list: p?.records?.map(mapMaker) ?? [], total: p?.total ?? 0 })
const brandPage = (p: Page<AnyObj>): PageResult<Brand> => ({ list: p?.records?.map(mapBrand) ?? [], total: p?.total ?? 0 })
const carPage = (p: Page<AnyObj>): PageResult<CarInfo> => ({ list: p?.records?.map(mapCar) ?? [], total: p?.total ?? 0 })
const rentalTypePage = (p: Page<AnyObj>): PageResult<RentalType> => ({ list: p?.records?.map(mapRentalType) ?? [], total: p?.total ?? 0 })
const customerPage = (p: Page<AnyObj>): PageResult<Customer> => ({ list: p?.records?.map(mapCustomer) ?? [], total: p?.total ?? 0 })
...
export const api = {
  ...
  makers: {
    ...
    searchPage: async (params: Partial<Maker> & { page?: number; size?: number } = {}) => {
      const { page: current = 1, size = 999, ...filters } = params
      return makerPage(await http.post<Page<AnyObj>>(`/rental/autoMaker/${current}/${size}`, makerSearchToApi(filters)))
    },
    search: async (params: Partial<Maker> & { page?: number; size?: number } = {}) => (await api.makers.searchPage(params)).list,
  ...
  brands: {
    searchPage: async (params: BrandSearchParams = {}) => {
      const { page: current = 1, size = 999, ...filters } = params
      return brandPage(await http.post<Page<AnyObj>>(`/rental/autoBrand/${current}/${size}`, brandSearchToApi(filters)))
    },
    search: async (params: BrandSearchParams = {}) => (await api.brands.searchPage(params)).list,
  ...
  cars: {
    searchPage: async (params: CarSearchParams = {}) => {
      const { page: current = 1, size = 999, ...filters } = params
      return carPage(await http.post<Page<AnyObj>>(`/rental/autoInfo/${current}/${size}`, carSearchToApi(filters)))
    },
    search: async (params: CarSearchParams = {}) => (await api.cars.searchPage(params)).list,
  ...
  rentalTypes: {
    ...
    searchPage: async (params: RentalTypeSearchParams = {}) => {
      const { page: current = 1, size = 999, ...filters } = params
      return rentalTypePage(await http.post<Page<AnyObj>>(`/rental/rentalType/${current}/${size}`, rentalTypeSearchToApi(filters)))
    },
    search: async (params: RentalTypeSearchParams = {}) => (await api.rentalTypes.searchPage(params)).list,
  ...
  customers: {
    searchPage: async (params: CustomerSearchParams = {}) => {
      const { page: current = 1, size = 999, ...filters } = params
      return customerPage(await http.post<Page<AnyObj>>(`/rental/customer/${current}/${size}`, customerSearchToApi(filters)))
    },
    search: async (params: CustomerSearchParams = {}) => {
      return (await api.customers.searchPage(params)).list
    },
  ...
  system: {
    ...
    bindRole: (userId: number, roleIds: number[]) => roleIds.length ? http.get<void>(`/rental/user/bind/${userId}/${roleIds.join(',')}`) : Promise.reject(new Error('旧后端不支持清空全部角色')),
    ...
    assignPermission: (roleId: number, ids: number[]) => ids.length ? http.get<void>(`/rental/role/${roleId}/${ids.join(',')}`) : Promise.reject(new Error('旧后端不支持清空全部权限')),
...
function mapUser(x: AnyObj): SysUser {
  return { id: x.id, username: x.username ?? '', realName: x.realname ?? '', nickname: x.nickname ?? '', gender: Number(x.gender) === 2 ? '女' : '男', deptId: x.deptId ?? 0, deptName: x.deptName ?? '', phone: x.phone ?? '', email: x.email ?? '', avatar: x.avatar ?? '', roleName: '', enabled: x.enabled ?? x.isEnabled ?? true, isAdmin: yn(x.isAdmin) }
}
...
function carToApi(c: Partial<CarInfo>) {
  return { id: c.id, autoNum: c.plateNo, makerId: c.makerId, brandId: c.brandId, infoType: c.carType === '电动车' ? 1 : c.carType === '混动车' ? 2 : 0, color: normalizeColor(c.color), displacement: Number(c.displacement) || 0, unit: c.displacementUnit, registrationDate: c.registerDate, mileage: c.mileage, rent: c.dayRent, pic: c.image, deposit: c.deposit, status: c.status ? carStatus.toApi(c.status) : undefined }
}
...
function makerSearchToApi(m: Partial<Maker>) { return { name: m.name || undefined, orderLetter: m.orderLetter || undefined } }
...
function rentalTypeSearchToApi(t: RentalTypeSearchParams) { return { typeName: t.name || undefined, lowDiscount: t.discountMin, highDiscount: t.discountMax } }
...
function maintainSearchToApi(m: MaintainSearchParams) { return { autoNum: m.plateNo || undefined, item: m.item || undefined, location: m.location || undefined, lowCost: m.costMin, highCost: m.costMax, lowMaintainTime: m.lowMaintainTime, highMaintainTime: m.highMaintainTime } }
...
function orderToApi(o: Partial<RentalOrder>) {
  return {
    ...
    customerTel: o.customerPhone,
    typeName: o.rentalType,
    typeDiscount: o.discount != null ? o.discount / 100 : undefined,
  }
}
function userToApi(u: Partial<SysUser>) {
  const data: AnyObj = { id: u.id, username: u.username, realname: u.realName, nickname: u.nickname, gender: u.gender === '女' ? 2 : 1, deptId: u.deptId, deptName: u.deptName, phone: u.phone, email: u.email, avatar: u.avatar, isEnabled: u.enabled }
  if (!u.id) data.password = u.password
  return data
}
function userSearchToApi(u: Partial<SysUser>) { return { username: u.username || undefined, realname: u.realName || undefined, nickname: u.nickname || undefined, gender: u.gender === '女' ? 2 : u.gender === '男' ? 1 : undefined, deptId: u.deptId && u.deptId > 0 ? u.deptId : undefined, phone: u.phone || undefined, email: u.email || undefined } }
