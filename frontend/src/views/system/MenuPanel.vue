...
async function openAdd() {
  if (!auth.hasPermission('sys:permission:add')) return ElMessage.warning('当前账号没有该操作权限')
...
async function openEdit(p: SysPermission) {
  if (!auth.hasPermission('sys:permission:edit')) return ElMessage.warning('当前账号没有该操作权限')
  await ensureParentOptions()
  isNew.value = false
...
async function save() {
  if (!auth.hasPermission(isNew.value ? 'sys:permission:add' : 'sys:permission:edit')) return ElMessage.warning('当前账号没有该操作权限')
  syncParentLabel()
  if (!form.parentLabel) return ElMessage.warning('请选择上级菜单')
  if (!form.name.trim()) return ElMessage.warning('请填写菜单名称')
  if (!form.type) return ElMessage.warning('请选择菜单类型')
  if (!form.permCode.trim()) return ElMessage.warning('请填写权限标识')
  if (form.type === 'button') form.componentPath = ''
  if (isNew.value) {
...
async function remove(p: SysPermission) {
  if (!auth.hasPermission('sys:permission:delete')) return ElMessage.warning('当前账号没有该操作权限')
  ...
  <div>
    <GlassCard padding="p-5">
      <div class="mb-4 flex items-center justify-between">
        ...
        <CircleIconButton v-if="auth.hasPermission('sys:permission:add')" icon="Plus" title="新增菜单" tone="brand" @click="openAdd" />
      </div>
      ...
      <el-table :data="store.permissions" class="glass-table" row-key="id" :indent="40" :tree-props="{ children: 'children' }" show-overflow-tooltip>
        ...
        <el-table-column label="上级菜单" min-width="130">
          <template #default="{ row }"><span class="text-ink-soft">{{ row.parentLabel || '—' }}</span></template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="110">
          <template #default="{ row }">
            <div class="flex gap-2">
              <button v-if="auth.hasPermission('sys:permission:edit')" class="flex h-8 w-8 items-center justify-center rounded-lg bg-editblue/10 text-editblue hover:bg-editblue/20" title="编辑" @click="openEdit(row)">
                <el-icon><Edit /></el-icon>
              </button>
              <button v-if="auth.hasPermission('sys:permission:delete')" class="flex h-8 w-8 items-center justify-center rounded-lg bg-danger/10 text-danger hover:bg-danger/20" title="删除" @click="remove(row)">
