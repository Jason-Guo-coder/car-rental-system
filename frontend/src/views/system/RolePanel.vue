...
const batch = useBatchDelete((ids) => store.removeRoles(ids), '角色')
...
function openAdd() {
  if (!auth.hasPermission('sys:role:save')) return ElMessage.warning('当前账号没有该操作权限')
  isNew.value = true
  Object.assign(form, { id: undefined, name: '', code: '', remark: '', createrId: undefined, permIds: [] })
  dialog.value = true
}
function openEdit(r: SysRole) {
  if (!auth.hasPermission('sys:role:edit')) return ElMessage.warning('当前账号没有该操作权限')
  isNew.value = false
  Object.assign(form, { ...r, permIds: [...r.permIds] })
  dialog.value = true
}
async function save() {
  if (!auth.hasPermission(isNew.value ? 'sys:role:save' : 'sys:role:edit')) return ElMessage.warning('当前账号没有该操作权限')
...
async function remove(r: SysRole) {
  if (!auth.hasPermission('sys:role:delete')) return ElMessage.warning('当前账号没有该操作权限')
...
async function openPerm(r: SysRole) {
  if (!auth.hasPermission('sys:role:bindPermission')) return ElMessage.warning('当前账号没有该操作权限')
  permTarget.id = r.id
  ...
  try {
    ...
    permTree.value = data.permissionList
...
async function savePerm() {
  ...
  if (!checked.length) return ElMessage.warning('旧后端不支持清空全部权限，请至少选择一个权限')
...
<template>
  <div class="role-panel-content">
    <FilterBar @search="doSearch" @reset="reset">
      ...
      <template #actions>
        <BatchDeleteButton v-if="auth.hasPermission('sys:role:delete')" :active="batch.active.value" @trigger="batch.trigger" @cancel="batch.exit" />
        <CircleIconButton v-if="auth.hasPermission('sys:role:save')" icon="Plus" title="新增角色" tone="brand" @click="openAdd" />
      </template>
    </FilterBar>
        ...
        <el-table-column label="操作" width="120">
          <template #default="{ row }">
            <div class="flex gap-2">
              ...
              <button v-if="auth.hasPermission('sys:role:bindPermission')" class="flex h-8 w-8 items-center justify-center rounded-lg bg-permteal/10 text-permteal hover:bg-permteal/20" title="权限分配" @click="openPerm(row)">
                <el-icon><Key /></el-icon>
              </button>
    ...
    <el-dialog v-model="permDialog" title="权限分配" width="440px" top="8vh" class="perm-dialog">
      ...
      <template #footer>
        ...
        <button v-if="auth.hasPermission('sys:role:bindPermission')" class="btn-brand ml-3 h-10 px-6 text-sm" @click="savePerm">确认分配</button>
      </template>
    </el-dialog>
  </div>
</template>
