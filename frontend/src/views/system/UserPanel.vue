...
const batch = useBatchDelete(async (ids) => {
  const normalIds = ids.filter((id) => !store.users.find((u) => u.id === id)?.isAdmin)
  if (normalIds.length !== ids.length) ElMessage.warning('超级管理员不能删除，已跳过')
  if (normalIds.length) await store.removeUsers(normalIds)
  return normalIds.length ? `已删除 ${normalIds.length} 条用户` : '未删除用户'
}, '用户')
...
function removeUser(u: SysUser) {
  if (u.isAdmin) return ElMessage.warning('超级管理员不能删除')
...
function openAdd() {
  if (!auth.hasPermission('sys:user:add')) return ElMessage.warning('当前账号没有该操作权限')
  isNew.value = true
  Object.assign(editing, { id: undefined, username: '', realName: '', nickname: '', gender: '男', deptId: 0, deptName: '', phone: '', email: '', avatar: '', password: '', roleIds: [], roleName: '', enabled: true })
  dialog.value = true
}
function openEdit(u: SysUser) {
  if (!auth.hasPermission('sys:user:edit')) return ElMessage.warning('当前账号没有该操作权限')
  isNew.value = false
  Object.assign(editing, { ...u, password: '', roleIds: [] })
  dialog.value = true
}
async function save() {
  if (!auth.hasPermission(isNew.value ? 'sys:user:add' : 'sys:user:edit')) return ElMessage.warning('当前账号没有该操作权限')
  ...
  if (!editing.deptId) return ElMessage.warning('请选择所在部门')
  editing.deptName = allDepts.value.find((d) => d.id === editing.deptId)?.name ?? ''
  if (isNew.value) {
    const { id: _omit, ...rest } = editing
    await store.addUser(rest)
    ElMessage.success('新增用户成功')
  } else {
    await store.updateUser(editing as SysUser)
...
async function uploadAvatar(options: UploadRequestOptions) {
  if (!auth.hasPermission(isNew.value ? 'sys:user:add' : 'sys:user:edit')) {
...
async function openBindRole(u: SysUser) {
  if (!auth.hasPermission('sys:user:bindRole')) return ElMessage.warning('当前账号没有该操作权限')
  roleTarget.id = u.id
  roleTarget.realName = u.realName
  roleTarget.roleIds = await store.userRoleIds(u.id)
  roleDialog.value = true
}
async function saveBindRole() {
  if (!auth.hasPermission('sys:user:bindRole')) return ElMessage.warning('当前账号没有该操作权限')
  if (!roleTarget.roleIds.length) return ElMessage.warning('旧后端不支持清空全部角色，请至少选择一个角色')
  await store.bindRole(roleTarget.id, roleTarget.roleIds)
  ...
  <div class="user-panel-grid grid grid-cols-[220px_1fr] gap-5">
    ...
    <div class="user-panel-content">
      <FilterBar @search="doSearch" @reset="reset">
        ...
        <template #actions>
          <BatchDeleteButton v-if="auth.hasPermission('sys:user:deleteBatch')" :active="batch.active.value" @trigger="batch.trigger" @cancel="batch.exit" />
          <CircleIconButton v-if="auth.hasPermission('sys:user:add')" icon="Plus" title="新增用户" tone="brand" @click="openAdd" />
        </template>
      </FilterBar>

      <GlassCard class="user-table-card" padding="p-5">
        <el-table :data="store.users" class="glass-table" show-overflow-tooltip>
          <el-table-column v-if="batch.active.value" label="" width="44">
          ...
          <el-table-column label="操作" width="120">
            <template #default="{ row }">
              <div class="flex gap-2">
                ...
                <button v-if="auth.hasPermission('sys:user:bindRole')" class="flex h-8 w-8 items-center justify-center rounded-lg bg-rolepurple/10 text-rolepurple hover:bg-rolepurple/20" title="绑定角色" @click="openBindRole(row)">
    ...
    <el-dialog v-model="roleDialog" title="绑定角色" width="380px" align-center>
      ...
      <template #footer>
        ...
        <button v-if="auth.hasPermission('sys:user:bindRole')" class="btn-brand ml-3 h-10 px-6 text-sm" @click="saveBindRole">确认绑定</button>
      </template>
    </el-dialog>
  </div>
</template>
