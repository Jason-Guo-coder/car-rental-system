...
const hasFullRoot = computed(() => !!root.value && root.value.parentId === 0 && !!root.value.children?.length)
const displayRoots = computed(() => (hasFullRoot.value ? root.value?.children ?? [] : store.deptTree))

const keyword = ref('')
const deptCards = (dept: SysDept) => {
  const out: { dept: SysDept; level: number }[] = []
  const walk = (nodes: SysDept[] = [], level = 0) => {
...
function openAdd(parentId?: number) {
  if (!auth.hasPermission('sys:dept:add')) return ElMessage.warning('当前账号没有该操作权限')
  isNew.value = true
  // 默认上级 = 所有部门（根），选它则创建公司级部门；传入 parentId 则预选该上级
  const pid = parentId ?? root.value?.id ?? 0
  Object.assign(form, { id: 0, parentId: pid, parentName: parentName(pid), name: '', orderNum: 0, phone: '', address: '' })
  dialog.value = true
}
function openEdit(d: SysDept) {
  if (!auth.hasPermission('sys:dept:edit')) return ElMessage.warning('当前账号没有该操作权限')
  isNew.value = false
...
async function save() {
  if (!auth.hasPermission(isNew.value ? 'sys:dept:add' : 'sys:dept:edit')) return ElMessage.warning('当前账号没有该操作权限')
...
function removeDept(d: SysDept) {
  if (!auth.hasPermission('sys:dept:delete')) return ElMessage.warning('当前账号没有该操作权限')
...
<template>
  <div class="space-y-5">
    ...
    <div class="flex items-center justify-between">
      <div>
        <p class="text-base font-semibold text-ink">{{ hasFullRoot ? root?.name : '部门管理' }}</p>
        <p class="mt-0.5 text-xs text-ink-muted">{{ displayRoots.length }} 个部门</p>
      </div>
      <div class="flex items-center gap-3">
        ...
        <CircleIconButton icon="Search" title="查询" tone="brand" @click="searchDept" />
        <CircleIconButton icon="RefreshLeft" title="重置" @click="resetDept" />
        <CircleIconButton v-if="auth.hasPermission('sys:dept:add')" icon="Plus" title="新增部门" tone="brand" @click="openAdd()" />
      </div>
    </div>

    <!-- 各公司分组 -->
    <GlassCard v-for="(company, ci) in displayRoots" :key="company.id" padding="p-6">
      <!-- 公司头 -->
      <div class="group mb-5 flex items-center gap-3">
        ...
        <div class="flex gap-1.5 opacity-0 transition-all group-hover:opacity-100">
          <button v-if="auth.hasPermission('sys:dept:add')" class="flex h-8 w-8 items-center justify-center rounded-full bg-brand/10 text-brand hover:bg-brand/20" title="新增下属部门" @click="openAdd(company.id)"><el-icon><Plus /></el-icon></button>
          <button v-if="auth.hasPermission('sys:dept:edit')" class="flex h-8 w-8 items-center justify-center rounded-full bg-editblue/10 text-editblue hover:bg-editblue/20" title="编辑" @click="openEdit(company)"><el-icon><Edit /></el-icon></button>
          <button v-if="auth.hasPermission('sys:dept:delete')" class="flex h-8 w-8 items-center justify-center rounded-full bg-danger/10 text-danger hover:bg-danger/20" title="删除" @click="removeDept(company)"><el-icon><Close /></el-icon></button>
        </div>
      </div>
      ...
      <div class="grid grid-cols-4 gap-4">
        <div
          v-for="item in deptCards(company)"
          :key="item.dept.id"
        ...
        >
          <div class="absolute right-2.5 top-2.5 flex gap-1 opacity-0 transition-all group-hover:opacity-100">
            <button v-if="auth.hasPermission('sys:dept:add')" class="flex h-6 w-6 items-center justify-center rounded-full bg-brand/10 text-brand hover:bg-brand/20" @click="openAdd(item.dept.id)"><el-icon :size="13"><Plus /></el-icon></button>
            <button v-if="auth.hasPermission('sys:dept:edit')" class="flex h-6 w-6 items-center justify-center rounded-full bg-editblue/10 text-editblue hover:bg-editblue/20" @click="openEdit(item.dept)"><el-icon :size="13"><Edit /></el-icon></button>
            <button v-if="auth.hasPermission('sys:dept:delete')" class="flex h-6 w-6 items-center justify-center rounded-full bg-danger/10 text-danger hover:bg-danger/20" @click="removeDept(item.dept)"><el-icon :size="13"><Close /></el-icon></button>
          </div>
          <p class="font-medium text-ink">{{ '　'.repeat(item.level) }}{{ item.dept.name }}</p>
          <div class="mt-2 space-y-1 text-xs text-ink-muted">
            <p class="flex items-center gap-1.5"><el-icon><Sort /></el-icon>排序 {{ item.dept.orderNum }}</p>
            <p class="flex items-center gap-1.5"><el-icon><Phone /></el-icon>{{ item.dept.phone }}</p>
            <p class="flex items-center gap-1.5 truncate"><el-icon><Location /></el-icon><span class="truncate">{{ item.dept.address }}</span></p>
          </div>
        </div>
      </div>
    </GlassCard>
