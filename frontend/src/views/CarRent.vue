...
const filters = reactive({
  makerId: undefined as number | undefined,
...
async function searchCars() {
  await carStore.search({
    ...
    lowRegistrationDate: applied.dateRange?.[0],
    highRegistrationDate: applied.dateRange?.[1],
    page: page.current,
    size: page.size,
  })
}
...
async function reset() {
  Object.assign(filters, { makerId: undefined, brandId: undefined, plateNo: '', carType: '', rentMin: undefined, rentMax: undefined, dateRange: [] })
...
onMounted(async () => {
  await Promise.all([
    makerStore.makers.length ? Promise.resolve() : makerStore.load(),
    rentalTypeStore.list.length ? Promise.resolve() : rentalTypeStore.load(),
  ])
  await searchCars()
})
...
function openRent(car: CarInfo) {
  if (!auth.hasPermission('busi:rental:rent')) return ElMessage.warning('当前账号没有该操作权限')
  selectedCar.value = car
  activeStep.value = 1
  Object.assign(form, { rentalType: '日租', name: '', phone: '', idCard: '', age: undefined, gender: '', birthDate: '', mileage: car.mileage, rent: car.dayRent, deposit: car.deposit })
  dialog.value = true
}
async function fillCustomerByPhone() {
  const phone = form.phone.trim()
  if (!phone) return
  const customer = await customerStore.byPhone(phone)
  if (!customer) return
  Object.assign(form, { name: customer.name, idCard: customer.idCard, age: customer.age, gender: customer.gender, birthDate: customer.birthDate })
}
async function confirmRent() {
  ...
  let customer = await customerStore.byPhone(form.phone)
  if (!customer) {
    ...
    customer = await customerStore.byPhone(form.phone)
  }
  ...
  <div class="paged-view-tight space-y-6">
    ...
    <FilterBar @search="doSearch" @reset="reset">
      <FilterField label="所属厂商">
        <el-select v-model="filters.makerId" placeholder="全部厂商" clearable class="w-36" @change="onMakerChange">
          <el-option v-for="m in makerStore.makers" :key="m.id" :label="m.name" :value="m.id" />
        </el-select>
      </FilterField>
      <FilterField label="所属品牌">
        <el-select v-model="filters.brandId" placeholder="全部品牌" clearable class="w-36">
    ...
    <div class="grid grid-cols-4 gap-4">
      <GlassCard v-for="(car, i) in carStore.list" :key="car.id" padding="p-3">
        <!-- 车辆图片 4:3 -->
        <div class="mb-2.5 flex aspect-[4/3] w-full items-center justify-center overflow-hidden rounded-xl bg-gradient-to-br text-ink-muted" :class="cardColors[i % 3]">
          <img v-if="car.image" :src="car.image" :alt="car.plateNo" class="h-full w-full object-cover" />
          <el-icon v-else :size="38"><Van /></el-icon>
        </div>
        ...
        <div class="mt-2.5 flex items-center justify-between">
          ...
          <button v-if="auth.hasPermission('busi:rental:rent')" class="btn-brand h-7 px-3 text-xs" @click="openRent(car)">立即出租</button>
        </div>
      </GlassCard>
      <el-empty v-if="!carStore.list.length" description="暂无可租车辆" class="col-span-4" />
    </div>
    ...
    <el-dialog v-model="dialog" title="办理出租" width="560px" align-center>
      <div v-if="selectedCar" class="space-y-5">
        ...
        <div class="flex items-center gap-3 rounded-card bg-white/60 p-3">
          <div class="flex h-12 w-16 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-gradient-to-br from-[#eef2ff] to-[#f5f0fb] text-ink-muted">
            <img v-if="selectedCar.image" :src="selectedCar.image" :alt="selectedCar.plateNo" class="h-full w-full object-cover" />
            <el-icon v-else :size="24"><Van /></el-icon>
          </div>
          <div class="flex-1">
        ...
        <div>
          ...
          <div class="grid grid-cols-2 gap-x-4 gap-y-3">
            ...
            <div>
              ...
              <el-input v-model="form.phone" placeholder="请输入电话" @blur="fillCustomerByPhone" />
            </div>
            ...
            <div>
              <label class="mb-1.5 block text-xs text-ink-label">起租里程</label>
              <el-input-number v-model="form.mileage" :min="0" class="!w-full" />
            </div>
            <div>
              <label class="mb-1.5 block text-xs text-ink-label">日租金额</label>
              <el-input-number v-model="form.rent" :min="0" class="!w-full" />
            </div>
            <div>
              <label class="mb-1.5 block text-xs text-ink-label">车辆押金</label>
              <el-input-number v-model="form.deposit" :min="0" class="!w-full" />
            </div>
          </div>
        </div>
      </div>
      ...
      <template #footer>
        ...
        <button v-if="auth.hasPermission('busi:rental:rent')" class="btn-brand ml-3 h-10 px-6 text-sm" @click="confirmRent">确认出租</button>
      </template>
    </el-dialog>
  </div>
</template>
