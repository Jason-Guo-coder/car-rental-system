...
onMounted(async () => {
  await dashboardStore.loadFinanceReport()
...
<template>
  <div class="space-y-6">
    <PageHeader title="财务报表" subtitle="经营收支分析与财务数据汇总">
      <template #actions>
        <div class="inline-flex rounded-full bg-white/60 p-1 shadow-glass-sm backdrop-blur">
          <router-link to="/finance" class="rounded-full bg-brand-gradient px-3.5 py-2 text-xs font-medium text-white shadow-glass-sm">财务首页</router-link>
          <router-link to="/finance/dayReport" class="rounded-full px-3.5 py-2 text-xs font-medium text-ink-soft transition hover:bg-white/70 hover:text-brand">财务日报</router-link>
          <router-link to="/finance/monthReport" class="rounded-full px-3.5 py-2 text-xs font-medium text-ink-soft transition hover:bg-white/70 hover:text-brand">财务月报</router-link>
        </div>
      </template>
    </PageHeader>
    ...
    <div class="grid grid-cols-4 gap-5">
      ...
      <StatCard label="在租订单" :value="dashboardStore.financeRentingOrders" suffix="单" icon="Tickets" tone="purple" />
    </div>
