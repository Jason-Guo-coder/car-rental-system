...
async function searchCustomers() {
  await store.search({ ...applied, gender: (applied.gender || undefined) as Customer['gender'] | undefined, page: page.current, size: page.size })
}
...
function openAdd() {
  if (!auth.hasPermission('busi:customer:add')) return ElMessage.warning('当前账号没有该操作权限')
  isNew.value = true
...
function openEdit(c: Customer) {
  if (!auth.hasPermission('busi:customer:edit')) return ElMessage.warning('当前账号没有该操作权限')
  isNew.value = false
  Object.assign(editing, c)
  dialog.value = true
}
function save() {
  if (!auth.hasPermission(isNew.value ? 'busi:customer:add' : 'busi:customer:edit')) return ElMessage.warning('当前账号没有该操作权限')
  if (!editing.name.trim()) return ElMessage.warning('请填写客户姓名')
  if (!editing.phone.trim()) return ElMessage.warning('请填写联系电话')
  if (!/^1[3-9]\d{9}$/.test(editing.phone)) return ElMessage.warning('联系电话格式不正确')
  if (!editing.idCard.trim()) return ElMessage.warning('请填写身份证号')
  if (!parseIdCard(editing.idCard.trim())) return ElMessage.warning('身份证号格式不正确')
  if (isNew.value) {
...
function remove(c: Customer) {
  if (!auth.hasPermission('busi:customer:delete')) return ElMessage.warning('当前账号没有该操作权限')
...
<template>
  <div class="paged-view-tight space-y-6">
    ...
    <div class="grid grid-cols-3 gap-5">
      <StatCard label="客户总数" :value="store.overviewTotal" suffix="人" icon="User" tone="brand" />
    ...
    <FilterBar @search="doSearch" @reset="reset">
      ...
      <template #actions>
        <BatchDeleteButton v-if="auth.hasPermission('busi:customer:delete')" :active="batch.active.value" @trigger="batch.trigger" @cancel="batch.exit" />
        <CircleIconButton v-if="auth.hasPermission('busi:customer:add')" icon="Plus" title="新增客户" tone="brand" @click="openAdd" />
      </template>
    </FilterBar>

    <GlassCard>
      <el-table :data="store.list" :class="['glass-table', { 'has-check': batch.active.value }]">
        <el-table-column v-if="batch.active.value && auth.hasPermission('busi:customer:delete')" label="" width="56">
        ...
        <el-table-column v-if="auth.hasPermission('busi:customer:edit') || auth.hasPermission('busi:customer:delete')" label="操作" width="110" fixed="right">
          <template #default="{ row }">
            <div class="flex gap-2">
              <button v-if="auth.hasPermission('busi:customer:edit')" class="flex h-8 w-8 items-center justify-center rounded-lg bg-editblue/10 text-editblue hover:bg-editblue/20" @click="openEdit(row)">
                <el-icon><Edit /></el-icon>
              </button>
              <button v-if="auth.hasPermission('busi:customer:delete')" class="flex h-8 w-8 items-center justify-center rounded-lg bg-danger/10 text-danger hover:bg-danger/20" @click="remove(row)">
    ...
    <FloatingPagination v-model:current-page="page.current" :total="store.total" :page-size="page.size" @change="changePage" />
    ...
    <el-dialog v-model="dialog" :title="isNew ? '新增客户' : '编辑客户'" width="520px" align-center>
      <div class="grid grid-cols-2 gap-x-4 gap-y-3">
        ...
        <div>
          ...
          <el-date-picker v-model="editing.birthDate" type="date" placeholder="请选择" value-format="YYYY-MM-DD" class="!w-full" disabled />
        </div>
        <div>
          <label class="mb-1.5 block text-xs text-ink-label">客户年龄</label>
          <el-input v-model.number="editing.age" placeholder="请输入年龄" disabled />
        </div>
        <div>
          <label class="mb-1.5 block text-xs text-ink-label">客户性别</label>
          <el-select v-model="editing.gender" placeholder="请选择" class="w-full" disabled>
            <el-option label="男" value="男" />
            <el-option label="女" value="女" />
          </el-select>
        </div>
