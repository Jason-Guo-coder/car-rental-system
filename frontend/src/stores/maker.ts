...
export const useMakerStore = defineStore('maker', {
  ...
  actions: {
    ...
    async searchBrands(keyword = '', makerId?: number) {
      const targetMakerId = makerId ?? this.activeMakerId
      if (!targetMakerId) {
        const brands = await api.brands.search({ name: keyword.trim() })
      ...
      const brands = await api.brands.search({ makerId: targetMakerId, name: keyword.trim() })
      this.activeBrands = brands.map((b) => ({ ...b, makerName: b.makerName || this.makers.find((m) => m.id === b.makerId)?.name || '' }))
    },
