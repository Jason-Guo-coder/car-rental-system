...
function buildWeeklyRevenue(orders: RentalOrder[]) {
  ...
  for (const order of orders) {
    const key = order.returnTime?.slice(0, 10)
    const day = days.find((item) => item.key === key)
    if (day) day.value += Number(order.paid ?? 0) / 1000
  }
  return days.map(({ label, value }) => ({ day: label, value: Number(value.toFixed(1)) }))
}
...
export const useDashboardStore = defineStore('dashboard', {
  state: () => ({
    ...
    fleetTotal,
    activeLeases: [] as typeof activeLeases,
    todos: [] as typeof todoItems,
  ...
  actions: {
    async loadFinanceReport() {
      ...
      this.financeRentingOrders = orders.filter((order) => order.status === 'renting').length
    },
    async load() {
      this.loading = true
      try {
        ...
        this.weeklyRevenue = buildWeeklyRevenue(orders)
        const rentingOrders = orders.filter((o) => o.status === 'renting')
        this.activeLeases = sample(rentingOrders, Math.min(6, rentingOrders.length)).map((o) => {
          const car = cars.find((item) => item.id === o.autoId || item.plateNo === o.plateNo)
          return {
            plate: o.plateNo,
            model: car?.brandName || o.rentalType,
            customer: o.customerName,
            start: o.startDate.slice(5, 10),
            end: o.endDate?.slice(5, 10) || '-',
            progress: 30,
            carInfo: car ? `${car.makerName || '未知厂商'} · ${car.carType}` : o.rentalType,
            mileage: car ? `${Number(car.mileage ?? o.startMileage).toLocaleString()}km` : `${Number(o.startMileage ?? 0).toLocaleString()}km`,
          }
        })
        this.todos = sample(violations.filter((v) => v.status === 'pending'), 5).map((v) => ({
          title: '违章待处理',
        ...
        this.financeRentingOrders = rentingOrders.length
      } finally {
        this.loading = false
      }
    },
